                  ...
                  if(AF>AN)  //Coarser fill material
                    {//System.out.println("AF>AN");
                     ...
                     if(hi<h)  //Intersecting Profile
                       {dx=xmaxn/(n-1);
                        xmax=xmaxn;
                        volume=B*W+0.6*AN*Math.pow(W,(5./3.))/
                           Math.pow((1-Math.pow((AN/AF),1.5)),(2./3.));
...
//                        System.out.println("yi, hi: "+yi+", "+hi);
                      ...
                      {dx=xmaxf/(n-1);
                       xmax=xmaxf;
                       volume=B*W+0.6*h*xmaxn*(Math.pow( ((W/xmaxn)+
                          Math.pow((AN/AF),1.5)),(5./3.))-Math.pow((AN/AF),1.5));
                       volume=(double)((int)volume);//truncate to whole numbers
                       for(int i=0;i<n;i++)
                        {x=i*dx;
                         hn[i]=AN*Math.pow(x,0.6666);
...
//            System.out.println("dx: "+dx);
            ...
            ANvalue="A_N= "+AN;
            AFvalue="A_F= "+AF;
            voltrun=((int)volume*100.0)/100;
            Vol="Volume = "+voltrun+" m^3/m";
            xmax=((int)xm*100.)/100.;
            Xmax="x_max= "+xmax;
              for(i=0;i<npts;i++)
               {depn[i]=bathn.h[i];
...
//          g.drawLine(0,amplitude+offset,length,amplitude+offset); //x axis
          ...
          g.drawString("Original Beach",10,totalh-50);
          g.setColor(Color.white);
          g.drawString("Ocean",length-75,iwave+offset+30);
          g.setColor(Color.black);
